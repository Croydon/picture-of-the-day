...
def nc_is_instance_reachable(core_config=config.config["core"], caching=True) -> bool:
    ...
    try:
        client = webdavclient({"webdav_hostname": config.config["core"]["nc_url"], "webdav_login": config.config["core"]["nc_username"], "webdav_password": config.config["core"]["nc_accesstoken"]})

        if client.check(f"/remote.php/dav/photos/{config.config["core"]["nc_username"]}/albums/"):
            if client.list(f"/remote.php/dav/photos/{config.config["core"]["nc_username"]}/albums/"):
